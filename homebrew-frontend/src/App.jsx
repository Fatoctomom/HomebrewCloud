...
export default function App() {
    ...
    const onUpload = async (file) => { await uploadFile(file); await refresh(); };
    const onDelete = async (name) => { if (confirm(`Delete "${name}"?`)) { await deleteFile(name); await refresh(); } };
    ...
    <div style={{maxWidth:800, margin:'2rem auto', padding:'0 1rem', fontFamily:'system-ui,sans-serif'}}>
      <h1>MiniCloud</h1>
      <p style={{opacity:0.75, marginTop:-8}}>Simple file browser for your Spring Boot backend</p>
      ...
      <div style={{border:'1px solid #eee', borderRadius:12, padding:'1rem'}}>
        ...
        {!loading && !err && files.map(f => (
          <FileRow key={typeof f === 'string' ? f : (f.name ?? f.filename)} file={f} onDelete={onDelete} onDownload={onDownload} />
        ))}
      </div>
    </div>
    );
